...
# use-after-free fixed in ecos_replay.nim (emscripten_exit_with_live_runtime)
...
# ecos_error_len(). Aborting keeps linear memory intact, and the page reads
# ecos_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "ecos_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_ecos_load_replay,_ecos_frame,_ecos_input,_ecos_packet_ptr,_ecos_packet_len,_ecos_error_ptr,_ecos_error_len,_ecos_stage_ptr,_ecos_stage_len
  """).replace("\n", " ")
)
