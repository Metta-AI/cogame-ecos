...
function stageNote() {
  ...
  try {
    var length = Module._ecos_stage_len ? Module._ecos_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._ecos_stage_ptr();
...
function runtimeError() {
  var length = Module._ecos_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._ecos_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._ecos_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._ecos_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._ecos_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._ecos_load_replay(pointer, length);
    });
...
function advance(frames) {
  ...
  try {
    ...
    for (var i = 0; i < count; i++) {
      if (Module._ecos_frame() < 0) throw new Error(runtimeError());
      ingestPacket();
    }
    postMessage({
      type: 'advanced',
...
importScripts('./wire_constants.js', './broadcast_core.js', './ecos_replay.js');
