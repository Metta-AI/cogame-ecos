(function () {
  ...
  function showFailure(error) {
    ...
    failed = true;
    console.error(error);
    // The two signals tools/ci/viewer_smoke.mjs and phase 60's
    // viewer-check.yml read. data-replay-loaded is set on the first drawn
    // frame below; this is its failure twin.
  ...
  function createCore(config) {
    ...
    function start() {
      ...
      try {
        worker = new Worker(workerUrl, { name: 'ecos-static-replay' });
